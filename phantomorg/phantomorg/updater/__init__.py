"""``po update`` — self-update from GitHub Releases, phantombot-style.

PhantomOrg ships as a git checkout (the ``bin/po`` / ``bin/phantomorg``
...
Repo coordinates are env-overridable (``PHANTOMORG_UPDATE_REPO=owner/name``)
...
# `po update` sees "Already on X" and never refreshes. The marker makes the
# pending refresh detectable and self-healing (see run_update).
PENDING_MARKER = ".pf-update-pending"
GITHUB_API = "https://api.github.com"
USER_AGENT = "phantomorg-update"
...
def repo_root() -> Path:
    """Absolute path of the PhantomOrg checkout this code lives in.

    ``updater/__init__.py`` sits three levels below the checkout root
    (``<root>/phantomorg/updater/``), so the root is the pyproject.toml
    ancestor — same convention ``cli._version`` relies on.
    """
    here = Path(__file__).resolve()
    for candidate in (here, *here.parents):
        if (candidate / "pyproject.toml").is_file() and (
            (candidate / "bin" / "po").is_file()
            or (candidate / "bin" / "pf").is_file()  # legacy alias
        ):
            return candidate
...
def apply_update(root: Path, tag: str) -> tuple[bool, str]:
    ...
    # merge and the venv refresh, the next `po update` repairs it (G).
    _write_pending_marker(root, tag)
    code, _, stderr = _git(root, "merge", "--ff-only", "--", tag)
    if code != 0:
        # No HEAD change happened — drop the marker so a later `po update`
...
def refresh_venv(root: Path) -> tuple[bool, str]:
    ...
    try:
        ...
        proc = subprocess.run(  # nosec B603 B607
            ...
            timeout=PIP_TIMEOUT,
            check=False,
            # The checkout root, not the caller's cwd: `po update` run from
...
def run_update(
    *,
    ...
    if not is_git_checkout(root):
        err.write(
            f"update failed: {root} is not a git checkout. `po update` needs "
    ...
    # this, PHANTOMORG_UPDATE_REPO pointing at repo B while the checkout's
    ...
    if pending:
        if check:
            out.write(
                ...
                "refresh pending — run `po update` to repair it.\n"
            )
        else:
