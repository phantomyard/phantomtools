"""
`po setup` — guided installation of PhantomOrg over an existing (or new)
...
def build_org_yaml(plan: SetupPlan) -> dict:
    ...
    persona, actors linked). Reuses the same structure `po new-org`
    writes, so `po validate` accepts it immediately."""
    roles: list[dict] = []
    actors: list[dict] = []
