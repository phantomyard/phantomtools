...
reference that previously only `po validate` detected after saving (a
...
def _require_org_doc_structure(doc: dict, org_path: Path) -> None:
    """Validate the minimal org.yaml structure `po setup` needs to reuse.
    ...
    if not isinstance(org, dict) or not isinstance(org.get("id"), str) or not org["id"]:
        raise click.ClickException(
            f"{org_path} is not a valid org.yaml for `po setup`: "
            "missing a non-empty organization.id. Fix the file or run "
            "`po validate` for details."
        )
        ...
        for entry in entries:
            if (
                ...
                raise click.ClickException(
                    ...
                    "or run `po validate` for details."
                )
...
def run_new_org_wizard() -> Path:
    ...
    template_choice = click.prompt(
        "Sector department template (see `po templates`)",
...
def run_add_role_wizard(org_path: Path) -> None:
    ...
    if not existing_depts:
        click.secho(
            ...
            "Use `po add-department` before adding a role.",
            fg="red",
        )
        raise SystemExit(1)
...
def run_add_actor_wizard(org_path: Path) -> None:
    ...
    if not existing_roles:
        click.secho(
            ...
            "Use `po add-role` before adding an actor.",
            fg="red",
        )
        raise SystemExit(1)
...
def run_setup_wizard(
    ...
    """
    `po setup` — guided installation over a phantombot personas root.
    ...
    click.echo("Next: `po validate` and `po build`, then `po deploy`.")
    return org_path
