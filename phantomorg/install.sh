#!/usr/bin/env bash
#
# install.sh — symlink PhantomOrg's CLI into your PATH.
#
# Two ways to run it:
#
...
#
# The symlinks point at bin/po and bin/phantomorg in the repo, so editing
# the repo takes effect on the next run — no pip reinstall needed. Re-run
# install.sh after moving the repo.
#
# Environment (all optional):
#   PREFIX                 install prefix (default $HOME/.local)
#   PHANTOMORG_REPO_URL  git URL to clone in standalone mode
#                          (default https://github.com/phantomyard/phantomtools.git)
#   PHANTOMORG_REPO_DIR  where to keep the clone in standalone mode
#                          (default $HOME/.local/share/phantomorg)
#
...
REPO_URL="${PHANTOMORG_REPO_URL:-https://github.com/phantomyard/phantomtools.git}"
REPO_DIR="${PHANTOMORG_REPO_DIR:-$HOME/.local/share/phantomorg}"
...
if [[ -z "$script_dir" || ! -x "$script_dir/bin/po" ]]; then
...
src_pf="$here/bin/po"
src_phantomorg="$here/bin/phantomorg"
...
if [[ ! -x "$src_phantomorg" ]]; then
    echo "install: $src_phantomorg not found or not executable" >&2
    exit 1
fi
