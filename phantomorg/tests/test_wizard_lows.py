...
adversarial review of `po setup` / mutations / compiler templates:

- L2: `po setup` on a malformed org.yaml crashes with a raw KeyError
...
from phantomorg.compiler.build import _env
from phantomorg.wizard import interactive, mutations
...
class TestL2SetupMalformedOrg(unittest.TestCase):
    """`po setup` on a malformed org.yaml must fail with a friendly
    ...
    def test_valid_org_structure_passes_check(self):
        ...
        with mock.patch(
            "phantomorg.wizard.interactive.find_personas_dirs", return_value=[]
        ):
...
class TestL4DirFsync(unittest.TestCase):
    ...
    def test_backup_org_file_fsyncs_parent_dir(self):
        with (
            mock.patch("phantomorg.wizard.mutations.os.replace") as mock_replace,
            mock.patch("phantomorg.wizard.mutations._fsync_dir") as mock_fsync,
        ):
    ...
    def test_fsync_dir_is_best_effort(self):
        ...
        with mock.patch("phantomorg.wizard.mutations.os.open", side_effect=OSError):
