"""Transactional rollback tests: `pf rollback` restores the system to
...
from phantomforge.deploy.target import DeployResult, archives_dir, deploy
...
class TestRollbackExecute(unittest.TestCase):
    ...
    def test_plan_rollback_raises_when_archive_missing(self):
        ...
        with tempfile.TemporaryDirectory() as t:
            ...
            from phantomforge.deploy.session import RollbackError
    ...
    def test_plan_rollback_refuses_without_trash_evidence(self):
        ...
        with tempfile.TemporaryDirectory() as t:
            ...
            with self.assertRaises(RollbackError) as ctx:
                plan_rollback(archive_root, target)
            self.assertIn("removed outside PhantomForge", str(ctx.exception))
    ...
    def test_plan_rollback_preserves_trash_evidence(self):
        ...
        proof. After >24h, running ``pf rollback`` must still produce the
    ...
    def test_plan_rollback_suffixed_session_id_matches_same_stamp_archive(self):
        ...
        skipped."""
        from phantomforge.deploy.session import begin_session
        ...
        with tempfile.TemporaryDirectory() as t:
            ...
            from phantomforge.deploy.session import _save_sessions
    ...
    def test_discard_session_removes_manifest_under_lock(self):
        ...
        from phantomforge.deploy import session as session_mod
        from phantomforge.deploy.session import (
            MANIFEST_NAME,
            _acquire_lock_file,
            _release_lock_file,
...
class TestManifestCorruption(unittest.TestCase):
    ...
    def test_load_sessions_raises_on_corrupt(self):
        from phantomforge.deploy.session import ManifestError
    ...
    def test_begin_session_quarantines_and_refuses(self):
        from phantomforge.deploy.session import begin_session
        from phantomforge.deploy.target import DeployError
        ...
        with tempfile.TemporaryDirectory() as t:
            ...
            quarantined = list(
                archive_root.glob(".phantomforge-manifest.json.corrupt-*")
            )
            self.assertEqual(len(quarantined), 1, str(ctx.exception))
            self.assertFalse((archive_root / ".phantomforge-manifest.json").exists())
    ...
    def test_empty_after_internals_keeps_root_on_corrupt(self):
        from phantomforge.deploy.session import _empty_after_internals
...
class TestDurableJournal(unittest.TestCase):
    ...
    def test_commit_transitions_to_committed(self):
        from phantomforge.deploy.session import begin_session, commit_session
...
class TestTransactionLock(unittest.TestCase):
    ...
    def test_transaction_lock_serializes_threads(self):
        ...
        from phantomforge.deploy.session import _transaction_lock
    ...
    def test_transaction_lock_file_created(self):
        from phantomforge.deploy.session import TRANSACTION_LOCK_NAME, _transaction_lock
    ...
    def test_deploy_and_rollback_use_transaction_lock(self):
        ...
        from phantomforge.deploy.session import TRANSACTION_LOCK_NAME
...
class TestSuffixExhaustion(unittest.TestCase):
    ...
    def test_archive_suffix_exhaustion_raises_not_nests(self):
        ...
        from phantomforge.deploy.target import (
            DeployError,
            _archive_stamp,
            archive_persona,
            archives_dir,
    ...
    def test_discard_suffix_exhaustion_raises_not_nests(self):
        ...
        from phantomforge.deploy.target import archives_dir
    ...
    def test_session_id_collision_gets_unique_suffix(self):
        ...
        from phantomforge.deploy.session import record_session
...
class TestStaleInternalsCleanup(unittest.TestCase):
    ...
    def test_tmp_files_always_collected(self):
        ...
        with tempfile.TemporaryDirectory() as t:
            ...
            stale = archive_root / ".phantomforge-manifest.json.12345.tmp"
    ...
    def test_trash_kept_with_corrupt_manifest(self):
        ...
        with tempfile.TemporaryDirectory() as t:
            ...
            archive_root = self._archive_root(tmp)
            trash = self._make_old_trash(archive_root)
            manifest = archive_root / ".phantomforge-manifest.json"
