...
from phantomforge.wizard.mutations import (
    DuplicateIdError,
    RemovalBlockedError,
    add_actor,
    add_department,
...
class TestDuplicateIdError(_AUCopyTestCase):
    ...
    existed, so a repeated `pf add-role --id ceo ...` silently created a
...
class TestAtomicSave(_AUCopyTestCase):
    ...
    def test_save_uses_atomic_replace(self):
        ...
        with mock.patch("phantomforge.wizard.mutations.os.replace") as mock_replace:
            add_role(
                self.org_path,
                "rol-atomic",
                "Rol Atomic",
    ...
    def test_save_failure_does_not_corrupt_live_file(self):
        ...
        with (
            mock.patch(
                "phantomforge.wizard.mutations.yaml.safe_dump",
    ...
    def test_backup_is_atomic_replace_and_leaves_no_temp(self):
        from phantomforge.wizard.mutations import backup_org_file

        with mock.patch(
            "phantomforge.wizard.mutations.os.replace", wraps=os.replace
        ) as mock_replace:
    ...
    def test_backup_failure_leaves_no_temp(self):
        ...
        with (
            mock.patch(
                "phantomforge.wizard.mutations.uuid4",
                side_effect=RuntimeError("simulated crash"),
            ),
            self.assertRaises(RuntimeError),
        ):
