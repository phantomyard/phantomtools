...
- F8: `pf build --only <unknown-id>` is a clean CLI error, no KeyError
...
from phantomforge.compiler.build import (
    write_if_changed,
    write_if_missing,
...
class TestAtomicWrites(unittest.TestCase):
    ...
    def _simulate_crash(self, writer, path, content):
        ...
        build_mod = sys.modules["phantomforge.compiler.build"]
...
class TestSymlinkRefusal(unittest.TestCase):
    ...
    def test_write_plain_if_changed_refuses_symlink(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            link = Path(tmp) / ".phantomforge.yaml"
            link.symlink_to(victim)
            with self.assertRaises(ValueError):
