...
restorable to its pre-deploy state via `pf rollback` (or, for pure
...
from phantomforge.deploy.target import DeployResult, archives_dir
...
class TestDeployCrashRecovery(unittest.TestCase):
    ...
    that `pf rollback` can reconcile from the filesystem."""
    ...
    def test_crash_between_archive_and_swap_then_rollback(self):
        ...
        with tempfile.TemporaryDirectory() as t:
            ...
            from phantomforge.deploy.target import deploy as deploy_fn
            ...
            with unittest.mock.patch(
                "phantomforge.deploy.target.os.replace",
...
class TestRollbackCrashRecovery(unittest.TestCase):
    ...
    def test_committed_retry_after_full_rollback_without_trash(self):
        ...
        personas are missing ... removed outside PhantomForge" (a false
...
class TestManifestWriteCrash(unittest.TestCase):
    ...
    def test_manifest_write_failure_keeps_old_manifest(self):
        ...
        with tempfile.TemporaryDirectory() as t:
            ...
            old = (archive_root / ".phantomforge-manifest.json").read_text(
                encoding="utf-8"
            )
...
class TestCompilerWriteCrash(unittest.TestCase):
    ...
    def test_atomic_write_crash_keeps_previous_content(self):
        from phantomforge.compiler.build import _atomic_write
        ...
        with tempfile.TemporaryDirectory() as t:
            ...
            with (
                unittest.mock.patch(
                    "phantomforge.compiler.build.os.replace",
