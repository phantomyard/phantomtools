"""Smoke tests for the CLI wrappers (bin/pf, bin/phantomforge, *.cmd).
...
class BinWrapperTest(unittest.TestCase):
    ...
    def test_pf_wrapper_help_exits_zero(self):
        proc = self._run("bash", str(REPO_ROOT / "bin" / "pf"), "--help")
        self.assertEqual(proc.returncode, 0, proc.stderr)

    def test_phantomforge_wrapper_help_exits_zero(self):
        proc = self._run("bash", str(REPO_ROOT / "bin" / "phantomforge"), "--help")
        self.assertEqual(proc.returncode, 0, proc.stderr)

    def test_wrapper_resolves_repo_even_via_symlink(self):
        # Simulate the install.sh layout: a symlink elsewhere pointing at
        # bin/pf — the wrapper must still resolve the real repo root.
        with tempfile.TemporaryDirectory() as tmp:
            link = Path(tmp) / "pf"
            link.symlink_to(REPO_ROOT / "bin" / "pf")
    ...
    def test_wrapper_works_with_venv_and_no_global_python3(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            bin_dir = root / "bin"
            bin_dir.mkdir()
            shutil.copy2(REPO_ROOT / "bin" / "pf", bin_dir / "pf")
            ...
            proc = self._run_env(env, bash, str(bin_dir / "pf"), "--help")
        self.assertEqual(proc.returncode, 0, proc.stderr)
    ...
    def test_wrapper_fails_without_python_and_without_venv(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for tool in ("dirname", "readlink"):
                ...
                (fakebin / tool).symlink_to(real)
            proc = self._run_env(
                {"PATH": str(fakebin)}, bash, str(bin_dir / "pf"), "--help"
            )
