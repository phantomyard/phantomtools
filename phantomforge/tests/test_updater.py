"""`pf update` tests: GitHub release discovery, version comparison and the
...
from phantomforge.updater import (
    EXIT_AVAILABLE,
    EXIT_ERROR,
    EXIT_OK,
    apply_update,
...
class ApplyUpdateTest(unittest.TestCase):
    ...
    def test_refuses_dirty_tree(self):
        ...
        with unittest.mock.patch(
            "phantomforge.updater.subprocess.run",
            return_value=subprocess.CompletedProcess(
                [], 0, stdout=" M phantomforge/cli.py\n", stderr=""
            ),
        ) as mock_run:
...
class RepoDiscoveryTest(unittest.TestCase):
    def test_repo_root_finds_pyproject_and_bin(self):
        ...
        self.assertTrue((root / "bin" / "pf").is_file())
...
class RefreshVenvTest(unittest.TestCase):
    ...
    def test_posix_layout_uses_bin_python(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                unittest.mock.patch("phantomforge.updater.os.name", "posix"),
                unittest.mock.patch(
                    "phantomforge.updater.subprocess.run",
                    return_value=subprocess.CompletedProcess(
                        [], 0, stdout="", stderr=""
    ...
    def test_windows_layout_uses_scripts_python_exe(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                unittest.mock.patch("phantomforge.updater.os.name", "nt"),
                unittest.mock.patch(
                    "phantomforge.updater.subprocess.run",
                    return_value=subprocess.CompletedProcess(
                        [], 0, stdout="", stderr=""
    ...
    def test_windows_layout_ignores_posix_venv(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with unittest.mock.patch("phantomforge.updater.os.name", "nt"):
                ok, message = refresh_venv(root)
        self.assertTrue(ok)
        self.assertEqual(message, "")
...
class RunUpdateTest(unittest.TestCase):
    ...
    def test_check_reports_available_exit_2(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                unittest.mock.patch(
                    "phantomforge.updater.is_git_checkout", return_value=True
                ),
                unittest.mock.patch(
                    "phantomforge.updater.remote_origin_repo", return_value="owner/repo"
                ),
                unittest.mock.patch(
                    "phantomforge.updater.read_local_version", return_value="0.4.19"
                ),
                unittest.mock.patch(
                    "phantomforge.updater.find_latest_release",
    ...
    def test_check_up_to_date_exit_0(self):
        with (
            ...
            unittest.mock.patch(
                "phantomforge.updater.read_local_version", return_value="0.5.0"
            ),
            unittest.mock.patch(
                "phantomforge.updater.find_latest_release",
                return_value=unittest.mock.MagicMock(
    ...
    def test_not_a_git_checkout_exit_1(self):
        with unittest.mock.patch(
            "phantomforge.updater.is_git_checkout", return_value=False
        ):
            code = run_update(check=True, root=Path("/tmp/x"))
    ...
    def test_no_remote_exit_1(self):
        with (
            ...
            unittest.mock.patch(
                "phantomforge.updater.remote_origin_repo", return_value=None
            ),
    ...
    def test_origin_mismatch_refused_exit_1(self):
        ...
        with (
            ...
            unittest.mock.patch(
                "phantomforge.updater.remote_origin_repo",
                return_value="owner/repo",
            ),
            unittest.mock.patch.dict(
                "os.environ",
                {"PHANTOMFORGE_UPDATE_REPO": "trusted-owner/phantomforge"},
            ),
        ):
            code = run_update(check=True, root=Path("/tmp/x"))
        self.assertEqual(code, EXIT_ERROR)
    ...
    def test_origin_missing_with_override_refused_exit_1(self):
        ...
        with (
            ...
            unittest.mock.patch.dict(
                "os.environ",
                {"PHANTOMFORGE_UPDATE_REPO": "owner/repo"},
            ),
        ):
            code = run_update(check=True, root=Path("/tmp/x"))
        self.assertEqual(code, EXIT_ERROR)
    ...
    def test_refresh_venv_uses_checkout_cwd(self):
        ...
        cwd — `pf update` from /tmp must install THIS repo."""
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
    ...
    def test_force_update_applies(self):
        with (
            ...
            unittest.mock.patch(
                "phantomforge.updater.read_local_version",
            ...
            unittest.mock.patch(
                "phantomforge.updater.apply_update",
                return_value=(True, "updated to v0.5.0"),
            ),
            unittest.mock.patch(
                "phantomforge.updater.refresh_venv", return_value=(True, "")
            ),
        ):
    ...
    def test_pending_marker_written_before_merge_and_cleared_on_merge_failure(
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # Merge fails: marker must be cleared so a later `pf update`
    ...
    def test_run_update_repairs_pending_marker(self):
        ...
        self-healed by the next `pf update` — even when already on the
        latest version."""
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            marker = root / ".pf-update-pending"
            ...
            with (
                ...
                unittest.mock.patch(
                    "phantomforge.updater.refresh_venv",
