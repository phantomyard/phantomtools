# PhantomForge — Technical specification
...
> Project name: **PhantomForge**. CLI: `phantomforge` (alias `pf`). Replaces "Forja" in all prior work.
...
PhantomForge is an engine that models **any organization** as departments, roles, actors, access policies, and communication rules — and compiles that into the files that an agent runtime (today, Phantombot) loads. Aquaponics United (AU) is the first real case, with concrete data already audited in this document (section 3).
...
| Reference | What it is | What it contributes to PhantomForge | What mistake it avoids |
|---|---|---|---|
...
| **FIPA-ACL / KQML** (90s-2000s standard, IEEE since 2005) | Agent communication language based on "performatives" (inform, request, agree, refuse...) with formal semantics | Confirms that **a small, closed vocabulary of message types** (not an endless list) is what makes an agent protocol usable | FIPA-ACL never achieved real adoption outside academia/defense precisely because of overly rich ontologies and verbose XML encoding — **PhantomForge limits the internal protocol to 5 message types**, not 12+ performatives |
| **CrewAI — hierarchical vs sequential process** | Agent framework with a "manager delegates to workers" mode | Confirms the **escalation matrix with explicit hierarchy** pattern (manager validates and redirects) | Production reports document that hierarchical delegation without an iteration limit (`max_iter`) can enter an infinite loop, and that only "manager" roles should have `allow_delegation=True` — **PhantomForge adopts this as `max_hops` and as a rule: only roles with reports_to null or with subordinates may re-escalate** |
| **RBAC vs ABAC vs PBAC** (NIST, IAM practices at AWS/enterprise) | Access control models: by fixed role, by dynamic attributes, or hybrid | Confirms that **a role for every exception** leads to "role explosion" (NIST documents organizations with thousands of roles). The validated solution is a **hybrid model**: RBAC for the base level (department/role) + per-actor exception attributes (ABAC) for cases like "Category 0 only for Salvador" | This is exactly the problem AU already has today: Category 0 lives "by hand" in Paco's SOUL. PhantomForge models it as an **actor exception attribute**, not as a new role |

**Conclusion of the external audit:** PhantomForge's design is not original in its parts — it is a deliberate combination of already-validated patterns, avoiding the two documented historical failures (FIPA-ACL: too much semantics, too little adoption; role explosion: too many roles, too little governance).
...
| # | Real finding in AU | Why it is a problem (with reference) | Solution in the PhantomForge spec |
|---|---|---|---|
...
**Explicit design decision:** PhantomForge **does not implement A2A's HTTP/JSON-RPC stack** — Phantombot agents are not independent, network-discoverable services; they are personas loaded by a single shared runtime. What is adopted from A2A and FIPA-ACL is the **vocabulary and discipline**, not the transport.
...
├── pyproject.toml
├── phantomforge/
│   ├── cli.py                  # entrypoint `phantomforge` / `pf`
│   ├── wizard/
│   │   ├── new_org.py
│   │   ├── add_department.py
│   │   ├── add_role.py
...
|---|---|---|
| `pf setup` | `--phantombot-dir`, `--org`, `--base` | Guided installation: detects existing personas, assigns each one a department + role (audit suggests, user confirms), optionally adds new personas, writes the org.yaml |
| `pf new-org` | `--id`, `--name`, `--sector`, `--lang` | Creates a minimal `organizations/<id>/org.yaml` |
| `pf add-department` | `--org`, `--id`, `--parent`, `--access-policy` | Adds an entry to `departments` |
| `pf add-role` | `--org`, `--id`, `--department`, `--reports-to`, `--access-level` | Adds an entry to `roles` |
| `pf add-actor` | `--org`, `--id`, `--role`, `--tools` | Adds an entry to `actors` |
| `pf build` | `--org`, `--only <actor_id>` (optional) | Runs the compiler over all actors or just one |
| `pf validate` | `--org` | Runs the full validator (schema + DAG + refs + budgets), exit code ≠0 on failure |
| `pf deploy` | `--from <dir>`, `--target <path>`, `--force`, `--prune`, `--yes` | Copies the compiled output to the runtime destination; asks a final `[y/N]` confirmation (skip with `--yes`); archives overwritten personas to `personas-archive/` |
| `pf deploy-all` | `--base`, `--dist-base`, `--target`, `--force`, `--prune`, `--yes` | Deploys every organization under `--base`; same confirmation/archive contract as `pf deploy`; records ONE aggregated session (a single `pf rollback` undoes the whole invocation) |
| `pf rollback` | `--target`, `--list`, `--yes` | Undoes the last deploy session and restores the system to exactly the state before it (see §7.2) |

### 7.1 Setup flow (`pf setup`)

`pf setup` is the guided installation path over a phantombot installation
...
5. **Apply.** A new org is written from the plan; an existing one is
   ...
   passes `pf validate` immediately.
...
- **Memories are untouched.** `pf setup` only writes the org.yaml; it
  never touches `MEMORY.md` or any persona content. `pf deploy` later
...
- **Persona archiving (phantombot-compatible).** Before an existing
  ...
  any PhantomForge-specific tooling.
...
- **Deploy session manifest.** Every `deploy`/`deploy-all` that changes
  ...
  `personas-archive/.phantomforge-manifest.json` (a dotfile phantombot
...
- **Transactional rollback (`pf rollback`).** Undoes the LAST session:
  ...
  - missing archives + no trash dir = they were removed OUTSIDE
    PhantomForge; refuse (the historical behavior, kept intact);
  ...
  Corrupt manifest: if `personas-archive/.phantomforge-manifest.json`
  ...
  - if the failure happens after the archives were consumed (e.g. the
    trash could not be removed), the next `pf rollback` becomes a
  ...
  (`.phantomforge-manifest.lock`), so two concurrent deploys cannot
  ...
  - before the first write to the target, `deploy`/`deploy-all` append
    ...
    process dies mid-deploy, `pf rollback` sees the `in_progress`
  ...
  - the whole deploy / deploy-all / rollback holds a *transaction
    lock* (`.phantomforge.lock` in the runtime dir, a dotfile
  ...
  no-ops on platforms without fcntl (e.g. Windows); PhantomForge
  ...
  Symlinks: PhantomForge never moves or copies through symlinks. A
...
Scope of backups: everything PhantomForge modifies. It never touches
`vault.sqlite`, `phantomchat.json` or memory indexes — those belong to
phantombot and are out of scope.

### 7.3 Phantomchat identity verification (`pf phantomchat-check`)
...
- **`actors[].npub`** (optional, NIP-19 bech32, validated incl.
  checksum at `pf validate`). Declaring it lets `pf build` warn when an
  actor has none, and lets the deploy side generate a contact directory
  (section 8).
- **`pf phantomchat-check --org <org.yaml>`** reads each persona's
  ...
  Usage: `pf phantomchat-check --org organizations/au/org.yaml
  [--personas-dir ~/.local/share/phantombot/personas] [--bin phantombot]`.

- **Build warning.** `pf build` emits a `no-npub` warning per actor
...
### 7.4 Telegram bot verification (`pf telegram-check`)
...
- **`actors[].telegram_bot`** (optional). At `pf validate` only the
  *shape* is checked (`@` + 5..32 chars of `[A-Za-z0-9_]`) — enough to
  catch typos, not enough to know whether the handle exists.
- **`pf telegram-check --org <org.yaml>`** reads the runtime config
  ...
  Usage: `pf telegram-check --org organizations/au/org.yaml
...
`MEMORY.md` uses `write_if_missing` instead: once created, the runtime keeps enriching it with durable facts, so PhantomForge must never regenerate it — not even with block merging.
...
- T0.3: `pf new-org` + `pf add-role` + `pf add-actor` (minimal wizard, 1 actor)
...
- T3.2: `pf deploy` with multi-organization support
...
3. **Name confirmation**: PhantomForge, CLI `pf`.
