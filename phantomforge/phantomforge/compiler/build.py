...
def write_plain_if_changed(path: Path, content: str) -> bool:
    ...
    merging. Used for the metadata file (.phantomforge.yaml), which has
...
def write_if_missing(path: Path, content: str) -> bool:
    ...
    PhantomForge must not touch it again on any later build.
    """
    if path.exists():
        _refuse_symlink(path)
        return False
...
def build_actor(
    ...
    # Source metadata — not runtime content, it's what lets `pf deploy`
    ...
    if spec.communication.human_channel or spec.communication.agent_channel:
        ...
        # Plain overwrite (like .phantomforge.yaml), NOT block merge: the
            ...
            warnings_out.append(
                {
                    ...
                    "message": (
                        ...
                        f"its NIP-19 npub in org.yaml or run `pf phantomchat-check` "
