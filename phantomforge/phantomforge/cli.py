"""
Entry point of the `phantomforge` / `pf` CLI (see section 7 of the spec:
...
def _meta_org_id(meta_path: Path) -> str | None:
    """Best-effort read of the organization_id from a .phantomforge.yaml.
...
def _reject_unresolved_in_progress(target: Path) -> None:
    ...
    if pending:
        ...
        click.secho(
            ...
            "Its target/archive state is unknown — run `pf rollback` to "
...
def main():
    """PhantomForge — generates AI agent organizations from a spec."""
...
@click.option(
    ...
    required=False,
    default=None,
    help="Sector department template (see `pf templates`)",
)
...
def setup_cmd(personas_root, org_path, base_dir):
    ...
    org.yaml. One pass — afterwards use `pf validate` / `pf build` /
    `pf deploy`.
    """
...
@click.option(
    ...
    default=None,
    type=_ExpandUserPath(file_okay=False, path_type=Path),
    help="Personas directory to verify against (default: $PHANTOMFORGE_TARGET_DIR or ~/.local/share/phantombot/personas)",
)
@click.option(
    "--bin",
    "phantomchat_bin",
...
def phantomchat_check_cmd(org_path, personas_dir, phantomchat_bin, as_json):
    ...
    if not result.ok:
        ...
        click.echo("Run `pf validate` for details.")
        raise SystemExit(1)
...
@click.option(
    ...
    default=None,
    type=_ExpandUserPath(file_okay=False, path_type=Path),
    help="Target personas directory (default: $PHANTOMFORGE_TARGET_DIR or ~/.local/share/phantombot/personas)",
)
@click.option(
    "--force",
    is_flag=True,
...
def deploy_cmd(compiled_dir, target, force, prune, assume_yes):
    """Copies the already compiled output to the runtime's personas directory.

    Existing personas that will be overwritten are first MOVED to
    personas-archive/ (phantombot's own backup convention) so the
    previous state can be restored with `phantombot import-persona`.
    """
    ...
    if not compiled_dir_path.is_dir():
        click.secho(
            ...
            f"{compiled_dir_path}. Run `pf build` (or `pf build-all`) first.",
            fg="red",
        )
        raise SystemExit(1)
    ...
    click.echo(
        "  overwrites : existing personas are archived to personas-archive/ first"
    )
    ...
    with _transaction_lock(effective_target):
        ...
        try:
            ...
            click.secho(
                ...
                "`pf rollback` to revert the partial deploy.",
                fg="red",
            )
            raise SystemExit(1)
        except OSError as e:
            ...
            click.secho(
                ...
                + "The deploy session is recorded as in_progress — run "
                "`pf rollback` to revert the partial deploy (it will "
        ...
        try:
            ...
            # and `pf rollback` would revert a deploy that actually
            ...
            click.secho(
                ...
                f"the manifest issue and re-run `pf deploy`, or run "
                f"`pf rollback` to revert this deploy.",
                fg="red",
            )
            raise SystemExit(1)
    ...
    if result.deployed or result.pruned or result.archived:
        click.secho(
            "Rollback available: `pf rollback` (restores the pre-deploy state)",
            fg="cyan",
        )
...
def deploy_all_cmd(base_dir, dist_base, target, force, prune, assume_yes):
    ...
    if not dist_base.is_dir():
        click.secho(
            ...
            f"Run `pf build-all` first (or pass --dist-base).",
            fg="red",
        )
        raise SystemExit(1)
    ...
    with _transaction_lock(effective_target):
        ...
        for org_id, _org_yaml in _iter_org_yaml_paths(base_dir):
            ...
            try:
                ...
                click.secho(
                    ...
                    "run `pf rollback` after this to reconcile the partial "
                ...
                click.secho(
                    ...
                    "in_progress; run `pf rollback` to clean up.",
                    fg="yellow",
                )
                raise SystemExit(1)
            click.secho(
        ...
        # `pf rollback` undoes everything this command changed.
        ...
        if mutation_failed:
            click.secho(
                ...
                "Run `pf rollback` to reconcile the partial deploy.",
                fg="yellow",
            )
            ...
            try:
                ...
                click.secho(
                    ...
                    f"The session is recorded as in_progress — either fix "
                    f"the manifest issue and re-run `pf deploy-all`, or run "
                    f"`pf rollback` to revert this deploy-all.",
                    fg="red",
                )
                raise SystemExit(1)
        else:
    ...
    # F3: a filesystem/mutation failure mid-deploy-all must exit non-zero
    # even when some orgs deployed fine — callers (CI) treat exit 0 as
    # "everything deployed". The in_progress session still records what
    # actually happened for `pf rollback`.
    if mutation_failed or ((failed or collided) and not ok):
        raise SystemExit(1)
...
def rollback_cmd(target, list_only, assume_yes):
    ...
    if not mine:
        ...
        mp = archive_root / ".phantomforge-manifest.json"
        if mp.is_file():
            click.secho(
                ...
                "(mv them back into the target), but `pf rollback` cannot help.\n",
                fg="red",
            )
        click.secho(
    ...
    with _transaction_lock(target_path):
        ...
        if fresh.session_id != plan.session_id:
            click.secho(
                ...
                "deploy was recorded). Nothing was changed — run `pf rollback` "
        ...
        try:
            ...
            click.secho(
                ...
                "`pf rollback` again to finish the cleanup.",
                fg="yellow",
            )
            raise SystemExit(1)
...
def remove_actor_cmd(org_path, actor_id, yes):
    ...
    click.secho(
        ...
        "delete its folder manually in the `pf deploy` target.",
        fg="yellow",
    )
...
def rename_actor_cmd(org_path, old_id, new_id, yes):
    ...
    click.secho(
        ...
        f"'{old_id}' until the next `pf build` + `pf deploy`.",
        fg="yellow",
    )
...
def import_audit_cmd(
    ...
    """
    Analyzes an existing persona folder (not generated by PhantomForge)
    ...
    click.secho(f"Applied to {against_org}. Run `pf validate` to confirm.", fg="green")
...
@click.option(
    ...
    required=False,
    default=None,
    help="Override the GitHub repo (owner/name). Default: remote.origin or $PHANTOMFORGE_UPDATE_REPO.",
)
def update_cmd(check, force, repo_override):
    """
    Self-update: fetch the latest PhantomForge release from GitHub and
    fast-forward this checkout to it (phantombot-style update cycle).

    The repo checkout is the source of truth (install.sh symlinks bin/pf
    ...
    repos or higher rate caps; PHANTOMFORGE_UPDATE_REPO overrides the
    repo to check.
    """
