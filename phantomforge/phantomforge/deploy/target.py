...
- Cross-organization collision: if the target already has an actor with
  ...
  `.phantomforge.yaml` the compiler leaves in each compiled actor), the
  deployment is rejected unless `force=True`.
- Unmanaged persona (hand-written SOUL): if the target already exists but
  has NO `.phantomforge.yaml` at all, it is treated as a collision — it
  may be a persona that never went through PhantomForge, and overwriting
...
with another's if they share an actor id. Without the second, `pf deploy`
...
# Default target when neither --target nor PHANTOMFORGE_TARGET_DIR is set.
# Phantombot's personas directory is used as the practical default, but
# PhantomForge itself is runtime-agnostic: any directory works.
...
ENV_TARGET_DIR = "PHANTOMFORGE_TARGET_DIR"


def default_personas_dir() -> Path:
    """Resolve the default personas directory.

    Priority: $PHANTOMFORGE_TARGET_DIR (if set and non-empty) →
...
_META_FILENAME = ".phantomforge.yaml"
...
# `phantombot import-persona`. PhantomForge reuses that exact convention
# so the backup it makes before overwriting is restorable with the
# runtime's own tooling (no PhantomForge-specific restore needed).
#
...
def archives_dir(personas_dir: Path) -> Path:
    """Sibling directory where phantombot (and PhantomForge) keep
...
def _assert_real_directory(path: Path, label: str) -> None:
    ...
    existing dir). Call before creating or using a directory PhantomForge
    treats as its own."""
    if path.is_symlink():
        raise DeployError(
...
class DeployResult:
    ...
    scopes.json is derived state (pure function of org.yaml), so it is
    never archived/restored by rollback — any rebuild regenerates it.
    It deliberately lives OUTSIDE the target tree (personas/) so the
    deploy transaction (target + personas-archive) stays byte-for-byte
    rollback-able; the rollback invariant must never depend on a
    derived artifact."""

    humans_written: bool = False
    """True when compiled HUMANS.md was transported to the data dir
    (target.parent). Same semantics as scopes_written: derived state,
    regenerated by any rebuild, never archived by rollback."""


def _write_scopes_file(compiled_dir: Path, target: Path, staging: Path) -> bool:
    """Transport compiled scopes.json (if any) into the data dir.

    Destination is ``target.parent / scopes.json`` (e.g.
    ``~/.local/share/phantombot/scopes.json``), NOT the target root:
    the rollback invariant requires the target tree to stay
    byte-for-byte restorable, and scopes.json is derived state that no
    rollback ever restores. The data dir is also where phantombot
    reads it from (next to memory.sqlite / memory-index/).

    Backward compatible: an old build without scopes.json deploys
    exactly as before (returns False). Symlink guards on both sides
    mirror the persona policy (refuse a planted link instead of
    following it); the copy goes through staging (same filesystem as
    the destination) so the final os.replace is atomic.

    Multi-org note: deploy-all calls this once per org, so the LAST
    org deployed wins at the data dir. The deferred phantombot issue
    will define the multi-org contract (e.g. per-org files); for the
    single-org deployments (the norm, e.g. aquaponics-united) this is
    exactly right.
    """
    src = compiled_dir / SCOPES_FILENAME
    if not src.exists():
        return False
    if src.is_symlink():
        raise DeployCollisionError(
            f"refusing to deploy: compiled {SCOPES_FILENAME} is a symlink"
        )
    dest = target.parent / SCOPES_FILENAME
    if dest.is_symlink():
        raise DeployCollisionError(
            f"refusing to deploy: data dir {SCOPES_FILENAME} is a symlink"
        )
    staging_scopes = staging / SCOPES_FILENAME
    shutil.copy2(src, staging_scopes)
    os.replace(staging_scopes, dest)
    return True


def _write_humans_file(compiled_dir: Path, target: Path, staging: Path) -> bool:
    ...
    so the rollback invariant never depends on this derived artifact.
    An org without a ``humans:`` block builds no HUMANS.md and deploys
    exactly as before (returns False).
    """
    src = compiled_dir / HUMANS_FILENAME
    if not src.exists():
        return False
    if src.is_symlink():
        raise DeployCollisionError(
            f"refusing to deploy: compiled {HUMANS_FILENAME} is a symlink"
        )
    dest = target.parent / HUMANS_FILENAME
    if dest.is_symlink():
        raise DeployCollisionError(
            f"refusing to deploy: data dir {HUMANS_FILENAME} is a symlink"
        )
    staging_humans = staging / HUMANS_FILENAME
    shutil.copy2(src, staging_humans)
    os.replace(staging_humans, dest)
    return True
...
def _assert_no_symlinks(actor_dir: Path) -> None:
    """Reject compiled actors that contain symlinks. PhantomForge
    ...
    for p in actor_dir.rglob("*"):
        if p.is_symlink():
            raise DeployCollisionError(
                ...
                f"contains a symlink ({p}). PhantomForge outputs plain "
...
def _preflight(
    ...
    for actor_dir in compiled_actor_dirs:
        ...
        if existing_org is None:
            # Target without PhantomForge metadata: it may be a
            ...
            if not force:
                collisions.append(
                    ...
                    f"PhantomForge metadata (.phantomforge.yaml) — it may "
            ...
            # a hand-assembled dir passed to `pf deploy --from`). Its
            ...
            if not force:
                collisions.append(
                    f"{actor_dir.name}: the build has NO PhantomForge metadata "
                    f"(.phantomforge.yaml) and the target already has this "
...
def deploy(
    ...
    try:
        ...
        # tree — the rollback invariant must never depend on derived
        # state. Derived state — no rollback archive needed.
        if staging is not None:
            result.scopes_written = _write_scopes_file(compiled_dir, target, staging)
            result.humans_written = _write_humans_file(compiled_dir, target, staging)
    finally:
