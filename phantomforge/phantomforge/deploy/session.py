"""
Deploy session manifest and transactional rollback (`pf rollback`).

Every deploy (or deploy-all) appends a *session* record to a manifest
kept inside the archive root:

    personas-archive/.phantomforge-manifest.json
...
``pf rollback`` then undoes the LAST session and returns the system to
...
TRANSACTION_LOCK_NAME = ".phantomforge.lock"
...
def record_session(
    ...
    *,
    ...
    ``pf rollback`` can warn when the spec has drifted since the deploy.
...
def begin_session(
    ...
    *,
    ...
    target/archive were modified, so ``pf rollback`` can reconcile the
...
class RollbackPlan:
    ...
    manual restore, an older PhantomForge version...). They are left
...
def plan_rollback(archive_root: Path, target: Path) -> RollbackPlan:
    ...
    if session.get("state") == "in_progress":
        ...
        for hit in sorted(archive_root.iterdir()):
            ...
            if name in planned_created:
                ...
                # restore, an older PhantomForge version...). Leave it
    ...
    if missing:
        ...
        #    archives were removed OUTSIDE PhantomForge (manual
        ...
        if present and not trash_evidence:
            raise RollbackError(
                ...
                "were removed outside PhantomForge. The rollback would be "
...
def execute_rollback(plan: RollbackPlan) -> RollbackResult:
    ...
    were consumed, the next `pf rollback` sees a cleanup-only plan and
    ...
    try:
        ...
        raise RollbackError(
            ...
            "changed, so you can retry `pf rollback` after fixing the "
            "cause."
        ) from e
...
def _execute_rollback_locked(plan: RollbackPlan, target: Path) -> RollbackResult:
    ...
    try:
        ...
        raise RollbackError(
            ...
            "`pf rollback` after fixing the cause — if the archived "
