...
left to `pf validate`; the remove/rename mutations do make their own
...
def _mutation_lock(org_path: Path):
    ...
    Two concurrent `pf add-*` / `pf setup` invocations (flag mode is
...
def add_role_and_actor(
    ...
    *,
    ...
    validation to `pf validate`, but this is a batch import path that
...
def rename_actor(org_path: Path, old_id: str, new_id: str) -> None:
    ...
    next `pf build` + `pf deploy` — this only changes the org.yaml, it
