...
all phantomforge work since v0.5.9 (Aug 11–12): per-actor memory scopes,
...
`humans:` block, and the new `pf telegram-check` anti-drift command.
...
- **`humans:` first-class registry** (`spec/model.py`, `spec/
  ...
  (id/name/role/telegram_user_id/npub). `pf build` generates an
  org-wide `HUMANS.md` registry (`__humans__` output); `pf deploy`
...
- **`pf telegram-check`** (new command, `compiler/telegram.py`):
...
- **`telegram_bot` shape validation** (`shape_validator.py`): handle
  must be `@` + 5..32 chars of `[A-Za-z0-9_]` at `pf validate` — the
...
- **`phantomchat.json` compiled from org.yaml** (bridge integration,
  96ff3c2).
- **npub schema + `pf phantomchat-check` + build warnings** (89f5ad7).
...
- **New test suite `tests/test_rollback_fault_injection.py` (698 lines):**
  ...
  - **Crash matrix (36 subtests, 2 scenarios):** scenario A = fresh
    ...
    turn, then retried through the real CLI (`pf rollback --yes`); the
...
- **#3 (MEDIUM): the wizard mutation lock was a no-op on Windows.** The
  ...
  `pf add-*` / `pf setup` writers could lose each other's mutations (the
...
- **G (MEDIUM): venv refresh could be lost between `git merge` and
  ...
  the venv stale with no way to notice — a later `pf update` saw
  ...
  the next `pf update` (even when already on the latest version) detects
...
- **H3 (HIGH): `pf update` supply-chain guard.** The update repository
...
- **C2 (MEDIUM-HIGH): `refresh_venv` now runs with `cwd=root`.** Running
  `pf update` from /tmp or another directory no longer pip-installs the
  wrong directory.
- **C3 (MEDIUM): POSIX wrappers no longer require a global python3.**
  `bin/pf` / `bin/phantomforge` prefer the repo `.venv` first; a portable
...
- `bin/pf` and `bin/phantomforge` used GNU-only `readlink -f` to resolve the
...
- New `bin/pf.cmd` and `bin/phantomforge.cmd` for Windows: resolve the repo
  from their own location, prefer `.venv\Scripts\python.exe`, else `python`,
  and delegate to `python -m phantomforge.cli`.
...
**`pf update` venv layout: Windows support.**

- `refresh_venv` used to hardcode `.venv/bin/python` (POSIX layout). On
  Windows a venv lives at `.venv/Scripts/python.exe`, so `pf update` on
...
**CI fix: bandit `# nosec` comments in `pf update`.**
...
- Bandit 1.9.4 does not accept explanatory text after the `# nosec` test
  ...
  and the CI `bandit -r phantomforge -q` step failed (exit 1) on
  `phantomforge/updater/__init__.py`.
...
**`pf update` hardening: remotes with embedded credentials.**
...
- `remote_origin_repo` now normalizes `https://user:token@github.com/owner/repo.git`
  ...
  as GitHub and `pf update` failed with "no GitHub remote found".
- New test case covering the credentials-embedded URL form.

## v0.5.0 — 2026-08-10

**`pf update` — self-update from GitHub Releases (phantombot-style update
cycle, Fase 5).**

### New command

- **`pf update [--check] [--force] [--repo owner/name]`** fetches the
  latest PhantomForge release from GitHub and fast-forwards the local
  checkout to it. PhantomForge ships as a git checkout (install.sh
  symlinks `bin/pf`), so an update is a git fast-forward to the released
  ...
  - Repo discovery: `remote.origin` → `$PHANTOMFORGE_UPDATE_REPO` →
...
- New `phantomforge/updater/` module: GitHub `/releases/latest` client,
...
- **`pf setup` validates the org.yaml structure (L2):** reusing a
  malformed org.yaml (missing `organization.id`, or a
  department/role entry without `id`) now fails with a friendly
  "not a valid org.yaml for `pf setup`" message instead of a raw
...
- **Wizard mode stops leaking tracebacks (L3):** `_run_wizard` now
  ...
  `pf new-org` over an existing org dir no longer dump a traceback.
...
- **Import audit atomicity (F5):** `pf import-audit --apply` applies
...
- **`--from`/`--target` paths expand `~` (F7):** all click `Path`
  ...
  `expand_user`) with `file_okay=False`, so `pf deploy --from ~/out
  --target ~/target` works as expected.
- **Interrupted deploys are honest (F8):** `pf deploy` interrupted by
  Ctrl-C now says "run `pf rollback`" (the session is committed so a
  rollback is available); `pf rollback` interrupted warns the target
  "may be partially applied".
- **`pf rename` asks for confirmation (F9):** destructive rename now
...
- **`import-audit` hardening (F6/F11):** unreadable/missing files are
  ...
  through on `--apply`.
- **Deploy-all exit code (F3):** `pf deploy-all` now exits 1 when a
...
- **Malformed metadata never crashes a deploy (F4):** `_read_meta` is
  ...
  `.phantomforge.yaml` files return `None` ("unknown origin") instead
...
- **Session commit/discard failures are friendly (F5):** if the
  ...
  the deploy failed.
- **Missing directories (F9):** `pf deploy-all` with a missing `--base`
...
- **`pf rollback` with nothing to roll back now exits 1** (was 0),
...
validator F1-F7; the two reviews cover the same `phantomforge/spec/`
module).
...
- **`pf build --only <unknown>` (F8):** friendly error listing the
...
- **Loader error contract (F1):** malformed YAML, invalid UTF-8, deep
  ...
  `OrgSpecError`; `pf build` shows a friendly error and `pf build-all`
...
Python 3.10 compatibility restored in the compiler and `pf setup`
create-new no longer overwrites an existing org.yaml without a backup.

### Fixed (HIGH): `datetime.UTC` broke Python 3.10

`phantomforge/compiler/build.py` used `datetime.UTC` (added in Python
...
### Fixed (HIGH): `pf setup` create-new silently overwrote an existing org.yaml
...
replaced with no backup — unlike `pf new-org`, which refuses. The
...
PhantomForge, refuse (historical behavior kept); missing + trash +
...
`.phantomforge-manifest.json`; the next deploy's `begin_session` loaded
...
`.phantomforge-manifest.json.corrupt-<stamp>` (never overwritten) and
...
acquiring `_transaction_lock`. Two concurrent PhantomForge processes
...
PhantomForge version) was "restored" into the target — or, if its
...
not be serialized there. PhantomForge targets Linux/phantombot runtimes;
...
the deploy records a committed session, so `pf rollback` can restore
...
in_progress session that `pf rollback` reconciles completely — proven
...
- `_save()` no longer opens `org.yaml` for a truncating write. It now
  ...
  `pf validate` / `pf build` / automation); readers see either the old
...
- **Deploy sessions are now durable (H1).** `pf deploy`/`deploy-all`
  ...
  with no trace: `pf rollback` detects the interrupted session and
...
- **Whole transactions are serialized (H2).** A new transaction lock
  (`.phantomforge.lock` in the runtime dir) is held by the entire
...
- **Rollback failure is now truly retryable (R1).** The session entry is
  ...
  personas were already restored, the next `pf rollback` becomes a
...
- **Concurrent deploys cannot lose sessions (R2).** `record_session`
  ...
  `.phantomforge-manifest.lock` (a dotfile, ignored by phantombot),
...
- **Symlinks are refused, never followed (R5).** `archive_persona`
  ...
  rejected; `pf rollback` refuses archived personas that are symlinks.
...
- **The confirmation now shows what will be replaced.** `pf rollback`
...
- **Archive paths are resolved at record time**, so a deploy invoked
  ...
  break `pf rollback` from another directory.
- **Corrupt manifest is reported.** `pf rollback` now tells the user
...
- **Deploy session manifest** (new): every `pf deploy` / `pf deploy-all`
  records a *session* in `personas-archive/.phantomforge-manifest.json`
...
- **`pf rollback`** (new): undoes the last deploy — archived personas are
  ...
  - `pf rollback --list` shows recorded sessions.
...
everything PhantomForge modifies is backed up first.**

- **Final confirmation before applying** (new): `pf deploy`, `pf deploy-all`
  and `pf setup` now show a summary of what they are about to do and ask
...
- **Personas are archived, never silently overwritten** (new): before
  `pf deploy` overwrites an existing persona (same organization, or
...
- **org.yaml is backed up before every mutation** (new): `add-*`,
  `remove-*`, `rename-*` and `pf setup` on an existing org write
...
**`pf setup` — guided installation over a phantombot installation.**

- New command: `pf setup [--phantombot-dir PATH] [--org org.yaml] [--base DIR]`.
  ...
  5. writes/mutates the org.yaml so `pf validate` accepts it immediately.
...
- `pf setup` only writes the org.yaml — it never touches MEMORY.md or any
...
- **`install.sh` (phantomyard tool convention)**: symlinks `bin/pf` and
  `bin/phantomforge` into `~/.local/bin` (or `$PREFIX/bin`) so the CLI is
...
- **`pf --version` fix**: now reads the version from the repo's
...
- **Runtime-agnostic deploy**: `PHANTOMFORGE_TARGET_DIR` env var to
...
- **Packaging fix**: the Jinja2 templates are now shipped in the wheel
  ...
  `pf build` finds `identity.j2`/`soul.j2`/`tools.j2`/`memory.j2`.
- **CLI UX**: `pf --version`; `new-org` only requires `--id` and
...
user wrote those values in, and it's not PhantomForge's
...
**Decision I made and why:** I'm leaving the CLI (`pf ...`, help
...
infrastructure with existing SOULs, never generated by PhantomForge:**
`pf deploy` only detected collisions by comparing `organization_id`
in `.phantomforge.yaml`. A hand-written SOUL doesn't have that
...
**Fix:** a destination that exists but has no `.phantomforge.yaml`
...
with hand-written `SOUL.md` (no metadata) in the destination → `pf deploy`
...
it shouldn't be):**
1. `pf import-audit --persona-dir <persona> --role-id <id> --against-org <org.yaml>`
   for each agent — captures only the structure (role, department,
   reports_to, tools), never the domain content.
2. `pf build --out <new-dir>` (never directly to the real destination) —
...
3. **Manual step, irreplaceable:** copy by hand what's valuable from
   ...
   regenerations.
4. Only then `pf deploy` from that already merged directory.
...
a repeated `pf add-role --id ceo` silently created a second entry
...
move on its own — you need `pf build` + `pf deploy` afterwards.
...
cross-references and the result still passed `pf validate`.
...
detected later with `pf validate`.
...
being saved and discovered later with `pf validate`.
...
**Gap found:** `pf new-org` in flags mode (`--template ngo`) did
...
(`click.Choice`, with "none" + the templates of `pf templates`) and
...
includes a metadata file (`.phantomforge.yaml`) with its
`organization_id`. `pf deploy` compares that id against the one
...
other's. New `pf list-orgs` command to list and validate at a glance
all organizations under `organizations/`.

**Sector templates (`pf new-org --template <sector>`):**
...
real organization, deliberately. New `pf templates` command to list
them.

**Import audit (`pf import-audit`):** analyzes an already existing
persona folder (not generated by PhantomForge — by hand, from a
...
**Gap found:** `pf new-org` resolved `{org_id}` with an f-string at
...
`pf new-org` or was written by hand: the generated SOUL.md always
...
PhantomForge at build time). `new_org.py` no longer resolves
...
See `phantomforge/compiler/blocks.py` and the tests in
...
recompile it from scratch with `pf build`, and move your manual
