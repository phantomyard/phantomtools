...
PhantomForge is a tool **for Phantombot**: the output is Phantombot's
persona format by design. The target directory is configurable — see
[Deploy target](#deploy-target).

See the full technical specification in [`docs/PhantomForge-spec.md`](docs/PhantomForge-spec.md)
...
CLI itself and `pf update` are OS-agnostic — Python 3.10+ and git are the
only requirements.
...
This fetches the repo and symlinks `pf` / `phantomforge` into `~/.local/bin`
...
`PHANTOMFORGE_REPO_DIR` to choose where the clone lives (defaults to
`~/.local/share/phantomforge`).

### Linux / macOS — symlink install (recommended, matches the phantomyard tool convention)

```bash
./install.sh                 # symlinks bin/pf and bin/phantomforge into ~/.local/bin
PREFIX=/usr/local ./install.sh   # or another prefix (may need sudo)
```

The repo stays the single source of truth: the symlinks point at `bin/pf` and
`bin/phantomforge` in this checkout, so editing the repo takes effect on the
...
```bat
setx PATH "%PATH%;C:\path\to\phantomforge\bin"
```

(or set it via System Properties > Environment Variables). Then use `pf` or
`phantomforge` from any shell — the `.cmd` wrappers resolve the repo root and
prefer `.venv\Scripts\python.exe` when the repo has a venv, else `python`.
Without the wrappers you can always run `python -m phantomforge.cli`.
...
pf setup --phantombot-dir ~/.local/share/phantombot/personas
...
#   -> org.yaml written; run `pf validate` + `pf build` + `pf deploy` next

# Or bring your own org.yaml and only assign personas to it:
pf setup --org organizations/my-org/org.yaml --phantombot-dir ~/.local/share/phantombot/personas

# Create an organization (optionally from a sector template)
pf new-org --id my-org --name "My Org" --sector ngo --lang en --template ngo
pf templates                          # list available templates

# Build the organization by hand (departments, roles, actors)
pf add-department --org organizations/my-org/org.yaml --id ops --name Operations --access-policy level-2
pf add-role       --org organizations/my-org/org.yaml --id lead --name "Team Lead" --department ops --access-level level-2
pf add-actor      --org organizations/my-org/org.yaml --id maria --role lead --tool email --tool drive

# Edit what's already there
pf rename-role   --org organizations/my-org/org.yaml --old-id lead --new-id team_lead
pf remove-actor  --org organizations/my-org/org.yaml --id maria
pf remove-role   --org organizations/my-org/org.yaml --id team_lead --cascade   # promotes subordinates, cleans escalation_matrix
pf remove-department --org organizations/my-org/org.yaml --id ops              # blocks while it still has roles

# Validate and compile
pf validate --org organizations/aquaponics-united/org.yaml
pf build --org organizations/aquaponics-united/org.yaml --out ./dist
pf deploy --from ./dist --target ~/.local/share/phantombot/personas/ --force   # --force only if you accept overwriting another organization

# Multi-organization
pf list-orgs --base organizations
pf build-all --base organizations --out ./dist            # compiles all to ./dist/<org_id>/
pf deploy-all --base organizations --dist-base ./dist --target ~/.local/share/phantombot/personas/ --prune

# Undo a deploy — the system returns to exactly the state before it
pf rollback --list        # show recorded deploy sessions
pf rollback               # confirm + undo the last deploy
pf rollback --yes         # non-interactive

# Import an existing persona (not generated by PhantomForge)
pf import-audit --persona-dir ./personas/marcos --role-id ops_lead \
...
`pf deploy` and `pf deploy-all` print a summary of what they are about to
...
options (not free text). `pf new-org` only needs `--id` and `--name` to run
...
actors:
  ...
  - id: alma
    role: project_lead
    npub: npub10dkp4yu0tfmra36qx35xa4apj28np3acfsrqtf07xqtqs3sxlvlsg3zkmq
...
```bash
pf validate --org organizations/my-org/org.yaml
pf build --org organizations/my-org/org.yaml --out ./dist
```
...
│   ├── .phantomforge.yaml # provenance — which org/role generated this persona
...
```bash
pf deploy --from ./dist --target ~/.local/share/phantombot/personas/
# summary printed, then [y/N] before anything is written
```

The generated files are **block-based**: everything PhantomForge owns lives
...
exactly as you wrote them. The CLI (`pf ...`, messages and errors) is in
...
- `--target <dir>` on `pf deploy` / `pf deploy-all`, or
- the `PHANTOMFORGE_TARGET_DIR` environment variable.

PhantomForge writes the persona files Phantombot expects and checks for
...
- **Final confirmation.** `pf deploy`, `pf deploy-all` and `pf setup`
...
- **Personas are archived before overwrite.** When `pf deploy` overwrites
...
- **Prune archives instead of deleting.** `pf deploy --prune` moves
  ...
  reversible.
- **Transactional rollback — `pf rollback`.** Every deploy records a
  *session* in `personas-archive/.phantomforge-manifest.json` (a dotfile
  Phantombot ignores). `pf rollback` undoes the last deploy and returns
  ...
  ```
  pf rollback --list     # show recorded sessions
  pf rollback            # confirm + undo the last deploy
  pf rollback --yes      # non-interactive
  ```

  `pf rollback` refuses to run when an archived persona is missing (an
  ...
  - **The session entry is dropped last.** If a rollback fails mid-way
    ...
    `pf rollback` again then becomes a *cleanup-only* pass that finishes
    the job.
  ...
  - **Deploys are durable: a crash can never leave an untraceable
    mutation.** Before touching the target, `pf deploy`/`deploy-all`
    ...
    `pf rollback` sees the interrupted session and *reconciles* it:
  ...
  - **Whole transactions are serialized.** A transaction lock
    (`.phantomforge.lock` in the runtime dir, a dotfile Phantombot
  ...
  - **Manifest-supplied paths are confined.** `pf rollback` validates
...
- **Symlinks are refused, never followed.** PhantomForge never moves or
...
PhantomForge is a **spec compiler**, not an agent runtime and not a
...
- **It does not run, host, or talk to the agents.** It only *generates*
  ...
  format) is what boots the persona; PhantomForge has no runtime of its
  own.
- **It does not invent a persona's accumulated history.** `MEMORY.md` is
  ...
  scaffolds. The agent's lived experience is the agent's own — PhantomForge
...
- **It does not wire the live channels.** The `communication` block records
  ...
  PhantomForge only compiles those values into the output; it does not
...
- **It does not understand arbitrary text.** `import-audit` is a text
  heuristic: it recognizes the patterns PhantomForge generates and a few
...
```
bin/                        # pf / phantomforge wrappers (symlinked by install.sh)
