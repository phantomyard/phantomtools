@echo off
rem phantommeet.cmd — run PhantomMeet from the repo checkout (Windows).
rem
rem Manual install (no installer on Windows yet): add this repo's bin
rem directory to your PATH, e.g.
rem   setx PATH "%PATH%;C:\path\to\phantommeet\bin"
...
rem stay inside the repo layout (bin\phantommeet.cmd).
setlocal
set "HERE=%~dp0"
