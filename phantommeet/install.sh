#!/usr/bin/env bash
#
# install.sh — symlink PhantomMeet's CLI into your PATH.
#
# Usage:
#   ./install.sh                 # symlink into ~/.local/bin
#   PREFIX=/usr/local ./install.sh   # symlink into /usr/local/bin (may need sudo)
#
# The repo stays the single source of truth: the symlinks point at bin/pm and
# bin/phantommeet in this checkout, so editing the repo takes effect on the
...
src_pm="$here/bin/pm"
src_phantommeet="$here/bin/phantommeet"
...
install_symlink "$src_pm" "$bindir/pm"
install_symlink "$src_phantommeet" "$bindir/phantommeet"
...
echo
echo "next steps:"
echo "  - smoke test:  pm --version   (and: pm --help)"
